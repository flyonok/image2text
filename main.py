...
# key_word = ['账', '钱', '价', '帐', '汇', '陈晓丽']
key_word = ['洪声', '得一', '林蝶', '李锡文']
img_save_dir = 'F:\\find_result\\backup'
...
def run_tesseract(filename, output_path, image_file_name):
    ...
    initCmd = ['tesseract --psm 3 -l chi_sim ']
    ...
    if not output_path:
        ...
        # initCmd.append("'")
        initCmd.append(image_file_name)
        # initCmd.append("' ")
        # initCmd.append("'")
        initCmd.append(temp_file)
        # initCmd.append("'")
        cmd = " ".join(initCmd)
        print(cmd)
        # subprocess.run(['tesseract ', '--psm 3 -l chi_sim ', image_file_name, temp_file],
    ...
    text_file_path = text_file_path.replace("\r\n", "")
    # subprocess.run(['tesseract --psm 3 -l chi_sim ', image_file_name, text_file_path],
    #                stdout=subprocess.PIPE,
    #                stderr=subprocess.PIPE)
    initCmd.append('"')
    initCmd.append(image_file_name)
    ...
    p.wait()
    try:
        with open('{}.txt'.format(text_file_path), 'r', encoding="utf8") as f:
                text = f.read()
                for word in key_word:
                    # if text.find(word) != -1 and (text.find('2018') != -1 or text.find('2019') != -1):
                    if text.find(word) != -1:
                        backup_name = os.path.basename(image_file_name)
                        shutil.copyfile(image_file_name, img_save_dir + "\\" + backup_name)
    ...
    if os.path.isdir(input_path):
        ...
        for ctr, filename in enumerate(os.listdir(input_path)):
            ...
            if filename.find(' ') != -1:
                ...
                os.rename(os.path.join(input_path, filename), os.path.join(input_path, newfile_name))
            image_file_name = os.path.join(input_path, newfile_name)
            print(run_tesseract(newfile_name, output_path, image_file_name))
            successful_files += 1
...
if __name__ == '__main__':
    ...
    rootPathColl = ["g:\\", "h:\\", "i:\\", "j:\\", "k:\\"]
    for root_path in rootPathColl:
        # root_path = "k:\\"
        output_path = "F:\\find_result\\txt"
        result1 = find('*.[Jj][Pp][Gg]', root_path)
        result2 = find('*.[pP][Nn][Gg]', root_path)
        print("find {} files".format(len(result1)))
        print("find {} files".format(len(result2)))
